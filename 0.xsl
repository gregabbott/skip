...
<xsl:stylesheet version="1.0" 
  ...
  <body>
    ...
    <!-- If(doc_has_headers){should make TOC} -->
    <xsl:variable name="has-headers">
      <xsl:call-template name="check-for-headers">
        <xsl:with-param name="text" select="$content"/>
      </xsl:call-template>
    </xsl:variable>
    <!-- Process lead content (content before first header) -->
    <xsl:if test="$has-headers = 'true'">
      ...
      <xsl:if test="normalize-space($lead-content) != ''">
        <section class="lead">
          <xsl:call-template name="process-lead-content">
            <xsl:with-param name="text" select="$lead-content"/>
          </xsl:call-template>
        </section>
      </xsl:if>
    </xsl:if>
    <!-- if page has headings make + add TOC-->
    <xsl:if test="$has-headers = 'true'">
    ...
    <article class="no_outline" tabindex="-1" accesskey="3">
      <xsl:choose>
        <xsl:when test="$has-headers = 'true'">
          ...
          <xsl:call-template name="process-sectioned-markdown">
            <xsl:with-param name="text" select="$content-from-first-header"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          <!-- No headers, process all content normally -->
          <xsl:call-template name="process-non-sectioned-markdown">
            <xsl:with-param name="text" select="$content"/>
          </xsl:call-template>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template name="extract-lead-content-recursive">
    ...
    <xsl:if test="string-length($remaining) > 0">
      ...
      <xsl:choose>
        <!-- Found first header, return accumulated content -->
  ...
  <!-- Extract content starting from first header -->
  ...
  <xsl:template name="skip-to-first-header">
    ...
    <xsl:if test="string-length($remaining) > 0">
      ...
      <xsl:choose>
        <!-- Found first header, return from here -->
        <xsl:when test="starts-with($line, '#')">
          <xsl:value-of select="$remaining"/>
        </xsl:when>
        <!-- Keep looking -->
  ...
  <!-- Process lead content (before first header) -->
  <xsl:template name="process-lead-content">
    <xsl:param name="text"/>
    <xsl:call-template name="process-lines-simple">
      <xsl:with-param name="remaining" select="$text"/>
  ...
  <!-- Process content without sections (docs without headings) -->
  <xsl:template name="process-non-sectioned-markdown">
    <xsl:param name="text"/>
    <xsl:call-template name="process-lines-simple">
      <xsl:with-param name="remaining" select="$text"/>
      <xsl:with-param name="in-code-block" select="false()"/>
  ...
  <xsl:template name="process-lines-simple">
    <xsl:param name="remaining"/>
    ...
    <xsl:if test="string-length($remaining) > 0">
      ...
      <xsl:choose>
        ...
        <xsl:when test="starts-with($line, '```')">
          ...
          <xsl:if test="normalize-space($paragraph-accumulator) != ''">
            <p>
              <xsl:call-template name="process-inline">
                <xsl:with-param name="text" select="$paragraph-accumulator"/>
              </xsl:call-template>
            </p>
          </xsl:if>
        ...
        <!-- Headers in lead content - process as regular headings -->
        <xsl:when test="starts-with($line, '#') and $in-code-block = false()">
          ...
          <!-- Process heading normally (no level shift in lead) -->
        ...
        <xsl:when test="substring($line, 1, 1) &gt;= '0' and substring($line, 1, 1) &lt;= '9' and contains(substring($line, 1, 5), '. ')">
          ...
          <xsl:choose>
            <xsl:when test="$current-level > $list-level or $list-type != 'ol'">
              <xsl:if test="$in-list = true() and $list-type != 'ol'">
                <xsl:call-template name="close-lists">
                  <xsl:with-param name="level" select="$list-level"/>
                  <xsl:with-param name="list-type" select="$list-type"/>
                </xsl:call-template>
        ...
        <xsl:when test="starts-with($line, '-') and substring($line, string-length(substring-before(concat($line, ' '), ' ')) + 1, 1) = ' '">
          ...
          <xsl:choose>
            <xsl:when test="$current-level > $list-level or $list-type != 'ul'">
              <xsl:if test="$in-list = true() and $list-type != 'ul'">
                <xsl:call-template name="close-lists">
                  <xsl:with-param name="level" select="$list-level"/>
                  <xsl:with-param name="list-type" select="$list-type"/>
                </xsl:call-template>
        ...
        <!-- Regular content line - accumulate for paragraph -->
        <xsl:otherwise>
          <xsl:variable name="new-accumulator">
            <xsl:choose>
    ...
    <!-- At end of content, flush any remaining paragraph -->
    <xsl:if test="string-length($remaining) = 0">
      <xsl:if test="normalize-space($paragraph-accumulator) != ''">
        <p>
          <xsl:call-template name="process-inline">
  ...
  <!-- Process markdown with sectioned structure-->
  <xsl:template name="process-sectioned-markdown">
    <xsl:param name="text"/>
    <xsl:call-template name="process-lines-with-sections">
      <xsl:with-param name="remaining" select="$text"/>
      <xsl:with-param name="in-code-block" select="false()"/>
      <xsl:with-param name="in-blockquote" select="false()"/>
      <xsl:with-param name="in-list" select="false()"/>
  ...
  <!-- Process lines with section management  -->
  <xsl:template name="process-lines-with-sections">
    <xsl:param name="remaining"/>
    <xsl:param name="in-code-block"/>
    <xsl:param name="in-blockquote"/>
    <xsl:param name="in-list"/>
    ...
    <xsl:if test="string-length($remaining) > 0">
      ...
      <xsl:choose>
        <!-- triple back tick block handling -->
        <xsl:when test="starts-with($line, '```')">
          <!-- Flush any accumulated paragraph -->
          <xsl:if test="normalize-space($paragraph-accumulator) != ''">
            <p>
        ...
        <xsl:when test="starts-with($line, '#') and $in-code-block = false()">
          ...
          <!-- Only manage section when outside of blockquotes -->
          <xsl:if test="$in-blockquote = false()">
            ...
            <!-- Close open sections if new heading level <= current -->
          ...
          <!-- Process heading with level shift outside blockquote, normal inside -->
          <xsl:call-template name="process-shifted-heading">
            <xsl:with-param name="line" select="$line"/>
            <xsl:with-param name="in-blockquote" select="$in-blockquote"/>
          </xsl:call-template>
          <!-- Continue processing -->
          <xsl:call-template name="process-lines-with-sections">
            ...
            <xsl:with-param name="list-level" select="0"/>
            <xsl:with-param name="list-type" select="'ul'"/>
            <xsl:with-param name="section-level">
              <xsl:choose>
                <xsl:when test="$in-blockquote = true()">
  ...
  <!-- Check if document contains any headers -->
  ...
  <!-- Close sections from one level down to another -->
  ...
  <!-- Process heading and shift level (# becomes h2, ## becomes h3, etc.) h1 single use for page NAME only | # == section -->
  <xsl:template name="process-shifted-heading">
    <xsl:param name="line"/>
    ...
    <xsl:choose>
      <xsl:when test="starts-with($line, '######')">
        <h6  class="h" >
          ...
          <a href="#{$heading-id}"  id="{$heading-id}" tabindex="0">#</a>
        </h6>
      </xsl:when>
      <xsl:when test="starts-with($line, '#####')">
        <h6  class="h" >
  ...
  <!-- Count list level based on number of dashes -->
  ...
  <!-- Get list item text after the dashes -->
  ...
  <!-- Open lists from one level to another -->
  ...
  <!-- Close lists from one level to another -->
  ...
  <!-- Process headings with auto-linking -->
  ...
  <!-- Generate TOC from headers with level classes, excluding those in code blocks and blockquotes -->
  ...
  <!-- Convert text to wiki URL format -->
  ...
  <!-- Shared link processing template 
  between`[md]()` and `[[wikilink]]` styles
  -->
  ...
  <!-- Process page URLs (add .xml if needed, handle spaces/case) -->
  ...
  <!-- Collect text until encounter specific marker -->
  ...
  <!-- Skip text until after a specific marker -->
  ...
  <!-- Parse table block with JSON-like array format -->
  ...
  <!-- Trim outer brackets from table content -->
  ...
  <!-- Safe substring-before: ignores delimiters inside quotes -->
  ...
  <!-- process cell content & handle misc line break types -->
  <xsl:template name="process-smart-cell-content">
    ...
    <xsl:choose>
      <xsl:when test="contains($processed-content, '&#10;&#10;')">
        ...
        <xsl:call-template name="process-lines-simple">
          <xsl:with-param name="remaining" select="$processed-content"/>
          <xsl:with-param name="in-code-block" select="false()"/>
          <xsl:with-param name="in-blockquote" select="false()"/>
          <xsl:with-param name="in-list" select="false()"/>
          <xsl:with-param name="list-level" select="0"/>
  ...
  <!-- Process lines with single line breaks -->
  ...
  <!-- Convert JSON escape sequences to characters -->
